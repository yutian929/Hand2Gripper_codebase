...
**This version removes contact modality input for ablation study.**
...
class DINOv2Backbone(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        B, C, H, W = x.shape
                ...
                # DINOv2 expects 224x224 or size divisible by 14
        if H != 224 or W != 224:
...
# ------------------------------
        # graph attention layer (Graph Attention)
# ------------------------------
...
class HandNodeEncoder(nn.Module):
    ...
    def forward(self, node_feats: torch.Tensor, img_feat_map: torch.Tensor,
        ...
        H = self.node_mlp(node_feats)  # [B, 21, D]
                ...
                # 2. Graph attention (model skeleton structure)
...
class PairDecoder(nn.Module):
    ...
    def forward(self, H: torch.Tensor):
        ...
        B, N, D = H.shape  # B=batch, N=21 keypoints, D=hidden dimension
                ...
                # dot product each keypoint with query vectors to get scores as left/right
        ...
        # S_lr[b,i,j] = H[b,i,:] @ W_lr @ H[b,j,:].T
                # represents compatibility degree of keypoint i as left, j as right in sample b
        ...
        comb = (
            ...
            S_lr                          # [B, N, N]
                ...
                # find max index after flattening
        comb_flat = comb.view(B, -1)             # [B, N^2]
        idx = torch.argmax(comb_flat, dim=1)     # [B]
        
                # recover (i_left, j_right) from flattened index
        i_left = idx // N      # [B]
        j_right = idx % N      # [B]
...
class Hand2GripperModel(nn.Module):
    ...
    Function:
            - map hand 3D keypoints to gripper left/right fingertips
    ...
    def _normalize_keypoints_xyz(kp3d: torch.Tensor) -> torch.Tensor:
        ...
        normal = Vh[:, 2, :]  # right singular vector corresponding to smallest singular value, i.e., normal vector [B,3]
                ...
                # Step 3: ensure normal vector points toward palm
    ...
    def _build_node_features(kp_xyz_norm: torch.Tensor, contact: torch.Tensor, 
        ...
        """concatenate [xyz | onehot | is_right] -> [B,21,25] (No Contact)"""
        B, N, _ = kp_xyz_norm.shape
        onehot = torch.eye(N, device=kp_xyz_norm.device).unsqueeze(0).repeat(B, 1, 1)
                # contact_f = contact.unsqueeze(-1) # remove contact
    ...
    def _read_color(self, color: np.ndarray) -> torch.Tensor:
        ...
        """
        if color.ndim != 3:
            raise ValueError(f"color ndim={color.ndim}, requires 3 dimensions")
        ...
        if color.shape[0] == 3 and color.ndim == 3:       # CHW
            ...
            raise ValueError(f"invalid color shape: {color.shape}, expect CHW or HWC with 3 channels")
    ...
    def _read_bbox(self, bbox: np.ndarray) -> torch.Tensor:
        ...
        if bbox.shape != (4,):
            raise ValueError(f"bbox shape should be (4,), got {bbox.shape}")
    ...
    def _read_keypoints_3d(self, keypoints_3d: np.ndarray) -> torch.Tensor:
        ...
        if kp.ndim == 2:
            if kp.shape != (21, 3):
                raise ValueError(f"keypoints_3d shape should be (21,3), got {kp.shape}")
            kp = kp[None, ...]                             # -> [1,21,3]
        elif kp.ndim == 3:
            if kp.shape[1:] != (21, 3) and not (kp.shape[0:3] == (1, 21, 3)):
                raise ValueError(f"unsupported keypoints_3d shape: {kp.shape}")
        else:
            raise ValueError(f"keypoints_3d should have 2 or 3 dimensions, got {kp.ndim}")
    ...
    def _read_contact(self, contact: np.ndarray) -> torch.Tensor:
        ...
        if c.ndim == 1:
            if c.shape != (21,):
                raise ValueError(f"contact shape should be (21,), got {c.shape}")
            c = c[None, ...]                               # -> [1,21]
    ...
    def _read_is_right(self, is_right: np.ndarray) -> torch.Tensor:
        ...
        if ir.ndim == 0:
            ...
            raise ValueError(f"is_right shape should be scalar or (1,), got {ir.shape}")
    ...
    def forward(self, img_crop: torch.Tensor, keypoints_3d: torch.Tensor,
        ...
        Args:
            ...
            keypoints_3d: [B, 21, 3]     hand 3D keypoints coordinates
            contact:      [B, 21]        contact probability (interface preserved but not used)
        ...
        """
                ...
                # img_feat_map: [B, H', W', D] spatial feature map, for Cross-Attention
                # img_emb: [B, D] global features, for output
        ...
        kp_xyz_norm = self._normalize_keypoints_xyz(keypoints_3d)
                # concatenate node features: [xyz | (contact-removed) | onehot | is_right]
